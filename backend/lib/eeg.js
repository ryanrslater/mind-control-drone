...
class Cortex {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      const checkHeadsetId = async () => {
        ...
        const found = response['result'].find(
          (item) => String(item['id']) === String(headsetId) && item['status'] === 'connected'
        );
        if (found) {
  ...
  train(profileName, trainingActions, numberOfTrain) {
    this.socket.on('open', async () => {
      ...
      await this.setupProfile(this.authToken, this.headsetId, profileName, status).then(
        (result) => {
      ...
      for (let trainingAction of trainingActions) {
        for (let numTrain = 0; numTrain < numberOfTrain; numTrain++) {
          ...
          console.log('\r\n');
          await self.trainRequest(self.authToken, self.sessionId, trainingAction, 'start');
          ...
          console.log('\r\n');
          await self.trainRequest(self.authToken, self.sessionId, trainingAction, 'accept');
        }
